...
class SGDRScheduler(Callback):
    ...
    def recovery_status(self, initial_epoch):
        ...
        if initial_epoch < self.cycle_length - 1:
            self.batch_since_restart = initial_epoch * self.steps_per_epoch
        else:

            acc = self.cycle_length
            while acc < initial_epoch + 1:
            ...
            if initial_epoch + 1 == self.next_restart:
                ...
                # on start
                if initial_epoch == self.next_restart - self.cycle_length:
                    self.batch_since_restart = self.steps_per_epoch
                else:
                    self.batch_since_restart = np.ceil(
                        ((initial_epoch + 1) / (self.next_restart - self.cycle_length))) * self.steps_per_epoch
